...
export default function App() {
    ...
    const repayContract = async (
        ...
        try {
            var rbalance = BigInt(
                await api.token().getBalance(repaymentaddress)
            );
    ...
    const cancelRequest = async (tokenid, refresh) => {
        ...
        if (v) {
            try {
                ...
                return alert(
                    "Request Cancelled",
                    "Your Earn Request was cancelled successfully!"
                );
            } catch (e) {
                loader(false);
                return error(e);
    ...
    const _processPayments = async () => {
        ...
        if (legacyPrincipalPayouts.hasOwnProperty(p)) {
            for (const canister in legacyPrincipalPayouts[p]) {
                ...
                await _processPaymentForCanister(
                    collections.find((a) => a.canister == canister)
                );
            }
        }
        loader(true, "Processing payments... (this can take a few minutes)");
        ...
        var _collections = collections.filter(
            (a) => canistersToProcess.indexOf(a.canister) >= 0
        );
        for (var j = 0; j < _collections.length; j++) {
            loader(
                true,
                "Processing payments... (this can take a few minutes)"
            );
    ...
    const _processPaymentForCanister = async (_collection) => {
        ...
        if (payments[0].length === 1)
            loader(true, "Payment found, processing...");
        ...
        for (var i = 0; i < payments[0].length; i++) {
            ...
            try {
                ...
                console.log(
                    "Payment extracted successfully",
    ...
    const wrapAndlistNft = async (token, loader, refresh) => {
        ...
        if (v) {
            ...
            if (loader)
                loader(true, "Creating wrapper...this may take a few minutes");
            try {
                var r = await extjs
                    .connect("https://boundary.ic0.app/", identity)
                    .canister(canister)
    ...
    const pawn = async (id, amount, reward, length, loader, refresh) => {
        ...
        try {
            ...
            return alert(
                "Request Received",
                "Your Earn Request was created successfully!"
            );
        } catch (e) {
            if (loader) loader(false);
            return error(e);
    ...
    React.useEffect(() => {
        if (identity) {
            ...
            if (
                legacyPrincipalPayouts.hasOwnProperty(
                    identity.getPrincipal().toText()
                )
            ) {
                for (const canister in legacyPrincipalPayouts[
                    identity.getPrincipal().toText()
                ]) {
                    if (
                        legacyPrincipalPayouts[
                            identity.getPrincipal().toText()
                        ][canister].length
                    ) {
